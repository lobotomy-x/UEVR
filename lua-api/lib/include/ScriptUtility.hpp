...
void set_property(sol::this_state s, void* self, uevr::API::UStruct* c, uevr::API::FProperty* desc, sol::object value,
    std::vector<std::unique_ptr<wchar_t[]>>* dynamic_strings = nullptr);
...
template <typename T> inline T read_t_struct(void* self, uevr::API::UStruct* c, size_t offset) {
    size_t size = 0;
...
template <typename T> inline void write_t_struct(void* self, uevr::API::UStruct* c, size_t offset, T value) {
    size_t size = 0;
    if (c->is_a(uevr::API::UScriptStruct::static_class())) {
        auto script_struct = reinterpret_cast<uevr::API::UScriptStruct*>(c);
...
template <typename T> void write_t(uevr::API::UObject* self, size_t offset, T value) {
    write_t_struct<T>(self, self->get_class(), offset, value);
}

template <typename T> T read_t(uevr::API::UObject* self, size_t offset) {
...
template <typename Elem> inline sol::object tarray_to_table(sol::this_state s, const uevr::API::TArray<Elem>& arr, const int32_t offset, const size_t name_hash) {
    if (arr.data == nullptr ) {
...
inline uevr::API::TArray<Elem>* table_to_tarray(sol::this_state s, sol::table value, int32_t capacity, const std::wstring name_hash) {
    if (arr.data == nullptr) {
        return sol::make_object(s, sol::lua_nil);
    }
    if (arr.count == 0) {
        ...
        for (const auto& [key, val] : value) {
            if (!key.is<int32_t>()) {
                throw sol::error("Invalid key type for array property (expected integer)");
            }
    ...
    for (int32_t i = 0; i < arr.count; ++i) {
        lua_arr[i + 1] = prop_to_object(s, arr.data[i], offset, name_hash, true);
...
} // namespace lua::utility} // namespace lua::utility