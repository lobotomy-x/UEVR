...
namespace lua::datatypes {
    void bind_vectors(sol::state_view& lua) {
        #define BIND_VECTOR3_LIKE(name, datatype, name2) \
            lua.new_usertype<name>(#name, \
                ...
                "update", [](name& v, datatype x, datatype y, datatype z) { \
                    v.x = x; v.y = y; v.z = z; \
                }, \
                ...
                sol::meta_function::addition, [](const name& lhs, datatype scalar) { return name{lhs.x + scalar, lhs.y + scalar, lhs.z + scalar}; }, \
                sol::meta_function::subtraction, [](const name& lhs, const name& rhs) { return lhs - rhs; }, \
                sol::meta_function::subtraction, [](const name& lhs, datatype scalar) { return name{lhs.x - scalar, lhs.y - scalar, lhs.z - scalar}; }, \
                sol::meta_function::multiplication, [](const name& lhs, datatype scalar) { return lhs * scalar; }, \
                sol::meta_function::multiplication, [](const name& lhs, const name& rhs) { return name{lhs.x * rhs.x, lhs.y * rhs.y, lhs.z * rhs.z}; }, \
                sol::meta_function::division, [](const name& lhs, datatype scalar) { return lhs / scalar; }, \
                sol::meta_function::division, [](const name& lhs, const name& rhs) { return name{lhs.x / rhs.x, lhs.y / rhs.y, lhs.z / rhs.z}; }, \
                sol::meta_function::new_index, [](const name& v, datatype w) -> name2 {return name2{v.x, v.y, v.z, w}; }, \
        ...
        #define BIND_VECTOR4_LIKE(name, datatype, datatype2) \
            lua.new_usertype<name>(#name, \
                ...
                sol::meta_function::addition, [](const name& lhs, const name& rhs) { return lhs + rhs; }, \
                sol::meta_function::subtraction, [](const name& lhs, const name& rhs) { return lhs - rhs; }, \
                sol::meta_function::addition, [](const name& lhs, datatype scalar) { return name{lhs.x + scalar, lhs.y + scalar, lhs.z + scalar, lhs.w + scalar}; }, \
                sol::meta_function::subtraction, [](const name& lhs, datatype scalar) { return name{lhs.x - scalar, lhs.y - scalar, lhs.z - scalar, lhs.w - scalar}; }, \
                sol::meta_function::multiplication, [](const name& lhs, datatype scalar) { return lhs * scalar; }, \
                sol::meta_function::multiplication, [](const name& lhs, const name& rhs) { return name{lhs.x * rhs.x, lhs.y * rhs.y, lhs.z * rhs.z, lhs.w * rhs.w}; }, \
                sol::meta_function::division, [](const name& lhs, datatype scalar) { return lhs / scalar; }, \
                sol::meta_function::division, [](const name& lhs, const name& rhs) { return name{lhs.x / rhs.x, lhs.y / rhs.y, lhs.z / rhs.z, lhs.w / rhs.w}; }, \
        ...
        #define BIND_VECTOR2_LIKE(name, datatype) \
            lua.new_usertype<name>(#name, \
                ...
                "to_vec3", [](const name& v) { return Vector3f{v.x, v.y, 0.0f}; }, \
                "to_vec4", [](const name& v) { return Vector4f{v.x, v.y, 0.0f, 0.0f}; }
