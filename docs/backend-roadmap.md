...
- [x] **Drag-and-drop from the MAIN tree view** — DONE 2026-07-02 (scheduled-run continuation). Two attach points: (1) every row in `ui_handle_actor` → Components list is now a drag source open-or-closed (same pattern as Attached/Overlapped lists); (2) the blue identity header in `ui_handle_object` is a drag source, so ANY inspected object — Common Objects → PlayerController/Pawn/World, Outer chains, camera-attach node — can be dragged onto caller slots/drop targets even when its row has no handle. Built clean (`UEVRBackend.dll` linked). Awaiting in-game verification: drag a Components row and the header onto a Function Caller slot.
