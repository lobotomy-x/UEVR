...
//  2025-06-02: [Docking] WM_DPICHANGED also apply io.ConfigDpiScaleViewports for main viewport instead of letting it be done by application
//  code. 2025-04-30: Inputs: Fixed an issue where externally losing mouse capture (due to e.g. focus loss) would fail to claim it again the
//  next subsequent click. (#8594) 2025-03-26: [Docking] Viewports: fixed an issue when closing a window from the OS close button (with
//  io.ConfigViewportsNoDecoration = false) while user code was discarding the 'bool* p_open = false' output from Begin(). Because we
//  allowed the Win32 window to close early, Windows destroyed it and our imgui window became not visible even though user code was still
//  submitting it. 2025-03-10: When dealing with OEM keys, use scancodes instead of translated keycodes to choose ImGuiKey values. (#7136,
//  #7201, #7206, #7306, #7670, #7672, #8468) 2025-02-21: [Docking] WM_SETTINGCHANGE's SPI_SETWORKAREA message also triggers a refresh of
//  monitor list. (#8415) 2025-02-18: Added ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress mouse cursor support. 2024-11-21: [Docking]
//  Fixed a crash when multiple processes are running with multi-viewports, caused by misusage of GetProp(). (#8162, #8069) 2024-10-28:
//  [Docking] Rely on property stored inside HWND to retrieve context/viewport, should facilitate attempt to use this for parallel contexts.
//  (#8069) 2024-09-16: [Docking] Inputs: fixed an issue where a viewport destroyed while clicking would hog mouse tracking and temporary
//  lead to incorrect update of HoveredWindow. (#7971) 2024-07-08: Inputs: Fixed ImGuiMod_Super being mapped to VK_APPS instead of
//  VK_LWIN||VK_RWIN. (#7768) 2023-10-05: Inputs: Added support for extra ImGuiKey values: F13 to F24 function keys, app back/forward keys.
//  2023-09-25: Inputs: Synthesize key-down event on key-up for VK_SNAPSHOT / ImGuiKey_PrintScreen as Windows doesn't emit it (same behavior
//  as GLFW/SDL). 2023-09-07: Inputs: Added support for keyboard codepage conversion for when application is compiled in MBCS mode and using
//  a non-Unicode window. 2023-04-19: Added ImGui_ImplWin32_InitForOpenGL() to facilitate combining raw Win32/Winapi with OpenGL. (#3218)
...
void ImGui_ImplWin32_Shutdown() {
    ...
    io.BackendFlags &=
        ~(ImGuiBackendFlags_HasMouseCursors | ImGuiBackendFlags_HasSetMousePos | ImGuiBackendFlags_HasGamepad |
            ImGuiBackendFlags_PlatformHasViewports | ImGuiBackendFlags_HasMouseHoveredViewport | ImGuiBackendFlags_HasParentViewport);
...
static void ImGui_ImplWin32_UpdateMouseData(ImGuiIO& io, ImGuiPlatformIO& platform_io) {
    ...
    if (is_app_focused) {
        ...
        // user) When multi-viewports are enabled, all Dear ImGui positions are same as OS positions.
        ...
        if (!io.WantSetMousePos && bd->MouseTrackedArea == 0 && has_mouse_screen_pos) {
            ...
            // corner of the app window) (This is the position you can get with ::GetCursorPos() + ::ScreenToClient() or WM_MOUSEMOVE.)
            // Multi-viewport mode: mouse position in OS absolute coordinates (io.MousePos is (0,0) when the mouse is on the upper-left of
            // the primary monitor) (This is the position you can get with ::GetCursorPos() or WM_MOUSEMOVE + ::ClientToScreen(). In theory
            // adding viewport->Pos to a client position would also be the same.)
    ...
    // If ImGuiBackendFlags_HasMouseHoveredViewport is not set by the backend, Dear imGui will ignore this field and infer the information
    // using its flawed heuristic.
    // - [X] Win32 backend correctly ignore viewports with the _NoInputs flag (here using ::WindowFromPoint with WM_NCHITTEST +
    // HTTRANSPARENT in WndProc does that)
    //       Some backend are not able to handle that correctly. If a backend report an hovered viewport that has the _NoInputs flag (e.g.
    //       when dragging a window for docking, the viewport has the _NoInputs flag in order to allow us to find the viewport under), then
    //       Dear ImGui is forced to ignore the value reported by the backend, and use its flawed heuristic to guess the viewport behind.
    // - [X] Win32 backend correctly reports this regardless of another viewport behind focused and dragged from (we need this to find a
...
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(
    HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam, ImGuiIO& io); // Doesn't use ImGui::GetCurrentContext()
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (ctx == NULL)
        return DefWindowProc(hWnd, msg, wParam,
            lParam); // unlike ImGui_ImplWin32_WndProcHandler() we are called directly by Windows, we can't just return 0.
    ...
    if (ImGui_ImplWin32_WndProcHandlerEx(hWnd, msg, wParam, lParam, io))
        ...
        case WM_MOUSEACTIVATE:
            ...
            break;
        case WM_NCHITTEST:
            // Let mouse pass-through the window. This will allow the backend to call io.AddMouseViewportEvent() correctly. (which is
            // optional). The ImGuiViewportFlags_NoInputs flag is set while dragging a viewport, as want to detect the window behind the one
            // we are dragging. If you cannot easily access those viewport flags from your windowing/event code: you may manually
            // synchronize its state e.g. in your main loop after calling UpdatePlatformWindows(). Iterate all viewports/platform windows
            // and pass the flag to your windowing system.
