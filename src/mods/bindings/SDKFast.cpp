...
#include "../VR.hpp" // world_to_screen_overlay: OverlayComponent remap + ImVec2 (transitive)
...
namespace
{
	...
	uevr::API::UObject* get_root_component(uevr::API::UObject* obj)
	{
		...
		if (actor == nullptr)
		{
			if (sdk_cast<sdk::AActor>(obj->get_outer())->get_root_component() == sdk_cast<sdk::USceneComponent>(obj))
			{
				return obj;
			}
		}
...
// VR-aware world→screen: same flat projection as world_to_screen above, then
// remapped onto the framework overlay quad so the point stays world-aligned
// while the UEVR UI is open (the same correction the built-in gizmo uses; see
// OverlayComponent::transform_world_aligned_to_overlay). Identity fallback to
// the plain flat result when no HMD is active or VR isn't initialized yet.
static glm::vec2 world_to_screen_overlay(uevr::API::UObject* player_controller, const glm::vec3& world_location)
{
	auto sp = world_to_screen(player_controller, world_location);

	auto vr = VR::get();
	if (vr != nullptr && vr->is_hmd_active())
	{
		const auto remapped = vr->get_overlay_component().transform_world_aligned_to_overlay(ImVec2{ sp.x, sp.y });
		sp.x = remapped.x;
		sp.y = remapped.y;
	}
...
void bindings::open_sdk_fast(sol::state_view& lua)
{
	...
	t["world_to_screen_overlay"] = &world_to_screen_overlay; // VR-aware (framework-overlay remap)
