...
inline void ui_object_label_compact(const std::string& full,
    ...
    const std::string shortl = prefix + UObjectHook::shorten_object_path(body);
...
bool render_universal_object_picker(const char* id_prefix, sdk::UObject*& slot,
    ...
    if (slot != nullptr && ImGui::BeginPopupContextItem("##uctx")) {
        ...
        if (allow_send_to_caller && ImGui::MenuItem("Send to Function Caller (as target)")) {
            UObjectHook::load_live_caller_target(slot);
        }
        ImGui::EndPopup();
    }
    ...
    if (auto picked = UObjectHook::render_object_picker_popup(popup_id.c_str(), st.filter,
...
void render_param_editor(ParamEditState& s, sdk::FProperty* prop, const std::string& prop_name_narrow, size_t name_hash) {
    ...
    case L"SoftClassProperty"_fnv: {
        ...
        ImGui::SameLine();
        {
            ...
            if (auto picked = UObjectHook::render_object_picker_popup(popup_id.c_str(),
...
// read_map_entries/read_set_entries: declared in UObjectHook.hpp (static members — see
// shorten_object_path's header comment for why), defined further below past the outermost
// anonymous namespace's close, alongside format_script_delegate/format_multicast_delegate.
...
std::string format_return_value(sdk::FProperty* prop, const uint8_t* params, size_t /*params_size*/) {
    ...
    case L"DelegateProperty"_fnv:
        return UObjectHook::format_script_delegate(in);
    ...
    case L"MulticastInlineDelegateProperty"_fnv:
        return UObjectHook::format_multicast_delegate(in);
    case L"MapProperty"_fnv: {
        std::vector<std::pair<std::string, std::string>> entries;
        const int num = UObjectHook::read_map_entries((sdk::FMapProperty*)prop, in, 16, entries);
    ...
    case L"SetProperty"_fnv: {
        ...
        const int num = UObjectHook::read_set_entries((sdk::FSetProperty*)prop, in, 16, entries);
...
// A static member (not a free function in the anonymous namespace above) so uobjecthook/Gizmo.cpp can
// call it too — internal linkage would make it invisible from a different translation unit. Must live
// OUTSIDE any anonymous namespace: MSVC rejects an out-of-line qualified member definition inside one
// (the file has NESTED anonymous namespaces, so this has to sit past the outermost one's close).
std::string UObjectHook::shorten_object_path(std::string_view full) {
    if (full.empty()) {
...
void UObjectHook::load_live_caller_target(sdk::UObject* obj) {
    if (obj == nullptr) return;
...
sdk::UObject* UObjectHook::render_object_picker_popup(const char* popup_id,
    ...
    if (ImGui::BeginListBox("##picker_list", ImVec2(420.0f, 280.0f))) {
        ...
        for (int32_t i = 0; i < count && shown < kCap; ++i) {
            ...
            const auto shortl = UObjectHook::shorten_object_path(narrow);
...
std::string UObjectHook::format_script_delegate(const uint8_t* d) {
...
std::string UObjectHook::format_multicast_delegate(const uint8_t* in) {
    const auto& arr = *(const sdk::TArrayLite<uint8_t>*)in;
...
int UObjectHook::read_map_entries(sdk::FMapProperty* mp, const uint8_t* base, int cap, std::vector<std::pair<std::string, std::string>>& out) {
...
int UObjectHook::read_set_entries(sdk::FSetProperty* sp, const uint8_t* base, int cap, std::vector<std::string>& out) {
